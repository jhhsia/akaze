...
inline void GetGaussianLinear( int k_size, float sigma, float* out)
{
    float sum = 0.0f;
    float half_inv_sig = 0.5f/(sigma*sigma);
    // open CV imp does not have this term in kernel
    // float mul_sig = half_inv_sig/M_PI;
    ...
    {
        int y_off = j - mid;
        float y_sqr = y_off*y_off;

        float val = (expf(-(  y_sqr )*half_inv_sig));
        *(out + j) = val;
        sum += val;
    }
...
inline void GetGaussianKernel( int k_size, float sigma, float* out)
{
    float sum = 0.0f;
    float half_inv_sig = 0.5f/(sigma*sigma);
    // open CV imp does not have this term in kernel
   // float mul_sig = half_inv_sig/M_PI;
    ...
    for (int i = 0; i < k_size; ++i)
    {
        ...
        {
            int y_off = j - mid;
            float x_sqr = x_off*x_off;
            float y_sqr = y_off*y_off;

            float val = (expf(-(x_sqr + y_sqr)*half_inv_sig));
            *(out + i + k_size*j) = val;
            sum += val;
        }
    }
